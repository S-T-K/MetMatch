...
//                    weightedsum/=listofSlices[i].getAdduct().getMZ()/1000000;
      ...
      if (bin>=0){
       ...
       //System.out.println(bin);
        ...
        int max = 0;
        int maxbin = 0;
        for (int i  = 1; i<mzbins.length-1; i++) {
            if (mzbins[i]>max) {
                max = mzbins[i];
                maxbin = i;
            }
...
//out.close();
    ...
    public void writeChangedFile() throws IOException {
        String content = readFile(file, StandardCharsets.UTF_8);
        StringBuilder file = new StringBuilder(content);
...
//print new file
PrintWriter out = new PrintWriter("./Output/MetMatch-corrected_" + this.file.getName());
out.println(newfile);
out.close();
